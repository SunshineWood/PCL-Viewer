  ...
  , bgColor(0, 0, 50)         // 初始化顺序要和声明的顺序一致
{
  ...
  view.reset(new pcl::visualization::PCLVisualizer(renderer, renderWindow,
                                                   "viewer", false));
  //设置背景颜色
  view->setBackgroundColor(double(bgColor.red()) / 255,
  ...
  if (isRBGA) {
    view->addPointCloud(cloudRGBA_, "cloud");
  } else {
    view->addPointCloud(cloud_, "cloud");
  }
...
{
  if (!cloud_->empty()) {
    view->setCameraPosition(0.5 * (p_min.x + p_max.x),
                               0.5 * (p_min.y + p_max.y),
                               p_max.z + 2 * maxLen,
                               0.5 * (p_min.x + p_max.x),
                               0.5 * (p_min.y + p_max.y),
...
{
  if (!cloud_->empty()) {
    view->setCameraPosition(p_min.x - 2 * maxLen,
                               0.5 * (p_min.y + p_max.y),
                               0.5 * (p_min.z + p_max.z),
                               p_max.x,
                               0.5 * (p_min.y + p_max.y),
...
{
  if (!cloud_->empty()) {
    view->setCameraPosition(p_max.x + 2 * maxLen,
                               0.5 * (p_min.y + p_max.y),
                               0.5 * (p_min.z + p_max.z),
                               p_max.x,
                               0.5 * (p_min.y + p_max.y),
...
PCLVisualizer::best_aligin()
{
  ...
  *cloud_tr = *cloud_RE; // 在cloud_tr中备份，以供显示
  ...
  if (isCloud2) {
    ...
    view->updatePointCloud(cloud_tr, cloud_tr_color_h, "cloud2");
  }

  view->updatePointCloud(cloud_in, cloud_in_color_h, "cloud");
  view->resetCamera();
  ui->openGLWidget->update();
}
...
PCLVisualizer::best_surface()
{
  *cloud_in = *cloud_;
  ...
  view->removePointCloud("cloud2");
  view->addPolygonMesh(mesh, "mesh");
  view->resetCamera();
  ui->openGLWidget->update();
}
...
PCLVisualizer::on_actionplaneSeg_triggered()
{
  ...
  *cloud = *cloud_;
  ...
  pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> single_color(
    cloud, 255, 255, 255);
  view->updatePointCloud<pcl::PointXYZ>(cloud, single_color, "cloud");
  ...
  for (int i = 0; i < n_piece; i++) {
    ...
    view->addPointCloud<pcl::PointXYZ>(planar_segment, plane_color, str);
    view->setPointCloudRenderingProperties(
